...
for d in dirs:
    if not isdir(join(DATA_DIR_2D, d)):
        ...
        scipy.io.loadmat(join(DATA_DIR_2D, d, f"{d}_NESTEROV.mat"), mdict=data["NAGD"])

        if not d == "alpine1":
            scipy.io.loadmat(join(DATA_DIR_2D, d, f"{d}_IPOPTData.mat"), mdict=data["IPOPT"])

        scipy.io.loadmat(join(DATA_DIR_2D, d, f"{d}_fminconData.mat"), mdict=data["MATLAB"])
...
####### # NESTEROV using 1e-4
        counter = 0
        for i in range(NUM_SHOTS):
            last_idx = data["NAGD"]["nesterov_last_frame"][i] - 1
            point = data["NAGD"]["nesterov_positions"][i, last_idx, :][0]
