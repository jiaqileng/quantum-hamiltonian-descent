...
# for instance in ["levy"]:
    ...
    sns.set_theme()
    fig = plt.figure()
    gs = fig.add_gridspec(4, len(snapshot_times), hspace=0.45)
    ...
    for col in range(n_plots):
        ...
        for row in range(4):
            ...
            ax.scatter(x_data, y_data, s=SCATTER_SIZE, c='darkslateblue', alpha=0.1)
            ax.set_xlim([0,1])
            ax.set_ylim([0,1])
    ...
    plt.savefig(f'./figures/alpha/{instance}_scatter.png', bbox_inches='tight', dpi=300)
