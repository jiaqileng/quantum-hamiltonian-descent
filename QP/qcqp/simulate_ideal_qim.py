...
def classical_simulation(instance):
	...
	# Load instance data
	instance_filename = join(benchmark_dir, f"instance_{instance}.npy")
	...
	T = [1e-3, 1e-2, 1e-1] # time unit: micro-second
	#T = [1] # time unit: micro-second
	...
	for i in range(r+1):
		for j in range(r+1):
			for k in range(r+1):
				for p in range(r+1):
					for q in range(r+1):
						coord_list.append([cells[i],cells[j],cells[k],cells[p],cells[q]])
	...
	for tf_micro in T:
		...
		sample_filename = join(benchmark_dir, f"advantage6_sim_rez{r}_T{tf}_sample_{instance}.npy")
